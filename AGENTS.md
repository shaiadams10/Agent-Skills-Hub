...
Local web app (Next.js) to **view and manage agent skills** installed on the user's machine (global + per-project).
...
│   │   ├── api/skills/          # DELETE skill folder
...
│       ├── motion/              # Motion tokens, presets, CSS classes (docs/MOTION.md)
...
- **Path audit prompt:** [`docs/PATH_AUDIT_PROMPT.md`](./docs/PATH_AUDIT_PROMPT.md) — copy-paste agent prompt to compare `skill-paths.ts` / `SCAN_PATHS.md` against each vendor's official docs (URLs embedded; no manual doc paste).
...
- [x] Unified motion system (`src/lib/motion/`, `src/components/motion/`, [`docs/MOTION.md`](./docs/MOTION.md))
